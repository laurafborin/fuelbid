...
export default function DistribuidoraLeilaoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function enviarLance(e: React.FormEvent) {
    ...
    await supabase.from('lances').insert({
      leilao_id: id,
      distribuidora_id: user.id,
      preco_litro: parseFloat(preco),
      prazo_entrega: prazo,
      observacao: obs,
    })
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {lances.map((lance, i) => (
                  <div key={lance.id} className={`flex items-center justify-between p-3 rounded-lg border ${i === 0 ? 'border-brand bg-brand/5' : 'border-gray-100'}`}>
                    ...
                    <div className="text-right">
                      <p className="font-bold">R$ {lance.preco_litro?.toFixed(3)}</p>
                      <p className="text-xs text-gray-500">{((leilao.preco_teto - lance.preco_litro) / leilao.preco_teto * 100).toFixed(1)}% abaixo do teto</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              ...
              <form onSubmit={enviarLance} className="space-y-3">
                <div>
                  ...
                  {preco && (
                    <p className={`text-xs mt-1 ${margem > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      Margem vs teto: {margem.toFixed(1)}% {margem > 0 ? '✓' : '✗'}
                    </p>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  <textarea value={obs} onChange={e => setObs(e.target.value)} rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:border-brand" />
                </div>
          ...
          {preco && (
            <div className="bg-white rounded-xl border border-gray-100 p-6">
              <h3 className="font-semibold text-sm mb-3">Simulação</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Valor Total</span><span className="font-medium">R$ {(parseFloat(preco) * leilao.volume_litros).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Economia p/ Posto</span><span className="font-medium text-green-600">R$ {((leilao.preco_teto - parseFloat(preco)) * leilao.volume_litros).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></div>
              </div>
            </div>
          )}
        </div>
